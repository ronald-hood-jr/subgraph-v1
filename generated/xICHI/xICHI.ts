...
export class xICHI extends ethereum.SmartContract {
  static bind(address: Address): xICHI {
    return new xICHI("xICHI", address);
  }

  Ichi(): Address {
    let result = super.call("Ichi", "Ichi():(address)", []);

    return result[0].toAddress();
  }

  try_Ichi(): ethereum.CallResult<Address> {
    let result = super.tryCall("Ichi", "Ichi():(address)", []);
    if (result.reverted) {
